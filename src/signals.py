...
def subtract_signal(t, signal, fit_params = 3):
    ...
    delta_signal = np.einsum('n,nj->j',
            coef, np.asarray([np.power(t, n) for n in range(fit_params)])) #(Nt)
...
def dphi_dop_chunked(t, mass, r0_vec, v_vec, d_hat, use_form = False, conc = 0, 
        use_chunk = False, chunk_size=10000, verbose = False):
    """
    ...
    if use_chunk == True:
        ...
        for i in range(num_chunks):
            ...
            dphi += dphi_dop(t, mass_c, r0_c, v_c, d_hat, use_form = use_form, 
                    conc = conc_c)
    else:

        dphi += dphi_dop(t, mass, r0_vec, v_vec, d_hat, use_form = use_form, 
                    conc = conc)

    return dphi

def dphi_dop_chunked_vec(t, mass, r0_vec, v_vec, use_form = False, conc = 0, 
        use_chunk = False, chunk_size=10000, verbose = False):
    """
    ...
    if use_chunk == True:
        ...
        for i in range(num_chunks):
            ...
            dphi_vec += dphi_dop_vec(t, mass_c, r0_c, v_c, use_form = use_form, 
                    conc = conc_c)
    else:

        dphi_vec += dphi_dop_vec(t, mass, r0_vec, v_vec, use_form = use_form, 
                    conc = conc)
...
def dphi_dop_vec(t, mass, r0_vec, v_vec, use_form = False, conc = 0):
    ...
    prefactor = const.yr_to_s*const.GN*mass/(
    ...
    if use_form:
        ...
        form_func = (form(r_cl/rv, conc)*np.heaviside(rv-r_cl, 0) + np.heaviside(r_cl-rv, 0)) # (N)
...
def dphi_dop(t, mass, r0_vec, v_vec, d_hat, use_form = False, conc = 0):
    ...
    if use_form:
        ...
        form_func = (form(r_cl/rv, conc)*np.heaviside(rv-r_cl, 0) + np.heaviside(r_cl-rv, 0))
